...
export default function EditModulePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const loadModule = async () => {
    try {
      setLoading(true);
      const response = await apiClient.get(`/content/modules/${resolvedParams.id}`);
      ...
      if (response.success && response.data) {
        ...
        setFormData({
          ...
          estimated_duration_minutes: moduleData.estimated_duration_minutes || 30,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.put(`/content/modules/${resolvedParams.id}`, formData);
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-2">Update module information</p>
      </div>
